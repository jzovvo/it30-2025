...
在真實的切版實戰中可能有超多層背景，到時 HTML 結構光用想像的都覺得可怕。

### 背景色延伸問題

![](./assets/troublesome-bg.png)

這是設計師蠻常見的處理照片手法：假設他想讓照片前面疊一層遮擋物（藍色），被遮擋的部分想要有某個部分露出（箭頭指的那個小小的地方），此時就會產生至少三層的圖層結構。

問題來了：照片想在 1 號位，但是藍色的遮擋物延伸到兩邊 2 號位，此時怎麼辦？這時候要馬照片用兩個綠框框個別裝起來，要馬將整個照片裝在綠框框中，然後在兩邊補上延伸的藍色，但如果遮擋不是單純的純色矩形就慘了。

這還只是這種情境最單純的情境，有此可知綠框框方案在很多時候會有些困難。

## 數學定位

![](./assets/remaining-space.png)

以數學的角度來看，如果我想讓 2 號位移到 1 號，其實只要將視窗多出來的空間的一半加在 2 號位上（藍線），那不就搞定了嗎！而多出來的空間就是視窗寬度（ `100vw` ）減去設計稿寬度，如此我們會得出一個公式：
...
跟綠框框效果是一樣的，但是又不用動到 `html` 結構，而缺點是需要在好幾個斷點去更新位置的公式，所以當背景條件很單純時，綠框框也許是個更簡單的選擇。
